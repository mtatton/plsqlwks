...
class AppEditorMixin:
    def prompt_search(self, direction: int = 1) -> None:
        query = self.prompt("Find", self.state.active_tab.search_query, strip=False)
        if query is None:
    ...
    def prompt_go_to_line(self) -> None:
        ...
        answer = self.prompt_text_box("Go to line", str(buffer.row + 1))
        if answer is None:
    ...
    def generate_sql_with_columns(self) -> None:
        ...
        table_name = self.prompt("Table", default_table)
        if table_name is None:
            self.state.status = "SQL generation cancelled"
            return
        table_name = table_name.upper()
        if not table_name:
            self.state.status = "No table selected"
            return
        columns = self.state.schema_columns.get(table_name)
        if columns is None:
            fingerprint = self.capture_editor_context()

            def columns_loaded(loaded: list[str]) -> None:
                self.state.schema_columns[table_name] = loaded
            ...
            self.start_db_operation(
                ...
                lambda db, progress: db.list_object_columns(table_name),
    ...
    def finish_generate_sql_with_columns(self, table_name: str, columns: list[str]) -> None:
        if not columns:
            self.state.status = f"No columns found for {table_name}"
            return
        options = list(SQL_GENERATOR_CHOICES)
        choice = self.pick("Generate SQL", options)
        if choice is None:
            self.state.status = "SQL generation cancelled"
            return
    ...
    def capture_editor_context(
        self,
    ) -> tuple[FileTab, str, int, int, tuple[int, int] | None, str]:
    ...
    def editor_context_is_current(
        self,
        fingerprint: tuple[FileTab, str, int, int, tuple[int, int] | None, str],
    ) -> bool:
        tab, text, row, col, selection_anchor, focus = fingerprint
        buffer = tab.buffer
        return (
            bool(getattr(self, "_db_completion_target_was_active", True))
    ...
    def default_generate_sql_table(self) -> str:
        entry = self.active_browser_entry() if self.state.focus == FOCUS_BROWSER else None
        browser_table = selected_browser_table_name(
            self.state.focus,
            entry,
        )
        if browser_table:
    ...
    def autocomplete_editor(self) -> None:
        ...
        if len(candidates) == 1:
            ...
            return
        choice = self.pick("Complete", [candidate.label for candidate in candidates])
        if choice is None:
    ...
    def load_completion_metadata_if_needed(self, context: CompletionContext) -> bool:
        ...
        cached_objects = {
            ...
            for object_type, names in self.state.browser_objects.items()
        }
        ...
        missing_columns = [
            name for name in table_names if name.upper() not in self.state.schema_columns
        ]
    ...
    def completion_candidates(self, context: CompletionContext) -> tuple[list[CompletionCandidate], str]:
        references = statement_table_references(context.statement)
        candidates: list[CompletionCandidate] = []
        if context.qualifier is not None:
            schema_objects = self.state.browser_objects if self.state.browser_loaded else empty_schema_object_groups()
            table_name = resolve_completion_qualifier(context.qualifier, references, schema_objects)
            if table_name is None:
                return [], ""
            columns, _column_error = self.completion_columns(table_name)
            candidates.extend(column_completion_candidates(columns, context.prefix, table_name))
    ...
    def completion_columns(self, object_name: str) -> tuple[list[str], str]:
        ...
        return self.state.schema_columns.get(normalized_name, []), ""
    ...
    def refresh_autocomplete_cache(self) -> None:
        ...
        def refresh_failed(exc: Exception) -> None:
            ...
            self.set_results(["ERROR refreshing autocomplete cache:", *wrap_error(exc)])
    ...
    def apply_completion(self, context: CompletionContext, candidate: CompletionCandidate) -> None:
        buffer = self.state.buffer
    ...
    def edit_key(self, key: int | str) -> None:
        ...
        if key in (KEY_SHIFT_LEFT, KEY_SHIFT_RIGHT, KEY_SHIFT_UP, KEY_SHIFT_DOWN):
            ...
            buf.move_line_start(extend=True)
        elif key == KEY_SHIFT_END:
            buf.move_line_end(extend=True)
        elif key == KEY_CTRL_SHIFT_HOME:
            buf.move_file_start(extend=True)
        elif key == KEY_CTRL_SHIFT_END:
            buf.move_file_end(extend=True)
    ...
    def copy_selection(self) -> None:
        ...
        provider = _legacy_ui_attr("copy_to_system_clipboard", copy_to_system_clipboard)(selected)
        if provider:
    ...
    def paste_clipboard(self) -> None:
        text, source = _legacy_ui_attr("paste_from_clipboard", paste_from_clipboard)(self.state.internal_clipboard)
        if not text:
    ...
    def transform_selection_case(self, transform: Callable[[str], str], status: str) -> None:
        if not self.state.buffer.transform_selection(transform):
            self.state.status = "No selection"
            return
        self.state.status = status

    def transform_selection_sql_code_case(self, transform: Callable[[str], str], status: str) -> None:
        selected = self.state.buffer.selection_range()
        if selected is None:
            self.state.status = "No selection"
            return
        transformed = transform_sql_code_in_selection(self.state.buffer.lines, selected, transform)
