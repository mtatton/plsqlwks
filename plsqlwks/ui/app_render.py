...
from .constants import _SYNTAX_COLOR_ATTRS, _SYNTAX_FALLBACK_ATTRS
from .display import *
from .keys import *
from .buffer import *
from .help import *
...
class AppRenderMixin:
    ...
    def draw_tab_bar(self, y: int, width: int) -> None:
        self.ensure_active_tab_visible(width)
    ...
    def draw_browser(self, y: int, height: int, width: int) -> tuple[int, int]:
        ...
        self.ensure_browser_selection_visible(body_h)
    ...
    def configured_explain_plan_attr(self, kind: str) -> int | None:
        enabled = getattr(self, "explain_color_kinds_enabled", set())
    ...
    def draw_result_grid(self, y: int, height: int, width: int, show_label: bool = True) -> tuple[int, int]:
        ...
        if visible_rows:
            self.ensure_selected_row_visible(visible_rows)
        widths = table_column_widths(result)
        self.ensure_selected_column_visible(widths, width)
    ...
    def draw_result_detail(self, y: int, height: int, width: int) -> tuple[int, int]:
        ...
        self.clamp_result_selection()
        self.ensure_selected_detail_field_visible(body_h)
        label = result_label(
            result,
            RESULT_ROW_DETAIL,
            self.state.focus == FOCUS_RESULTS,
    ...
    def draw_status(self, y: int, width: int) -> None:
        text = active_operation_status(self.state) if self.db_operation_active() else self.state.status
